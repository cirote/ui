...
<div class="form-group @error($item) text-danger @enderror">
    ...
    <div class="input-group">
        <input  wire:model="{{ $item }}" 
                ...
                class="form-control @if($type == 'date') datetimepicker-input @endif @error($item) is-invalid @enderror" 
                @if($type == 'date') data-target="#{{ $item_sin_punto }}" @endif  
                @if($type == 'number') onkeypress="return /[.\-0-9]/i.test(event.key)" @endif >
